...
function color_table_init() {
	...
	if (variable_global_exists("mix_map")) ds_map_destroy(global.mix_map);
    global.mix_map = ds_map_create();
    for (var i = 0; i < array_length(global.mix_table); i++) {
        var cm = global.mix_table[i];
        ds_map_add(global.mix_map, cm.value, cm);
    }
