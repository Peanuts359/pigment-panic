...
if (p < edge_grow) {
	...
	if (!reached_max) {
		drop_stacks = 2;
		reached_max = true;
		show_debug_message("maxed!");
	}
    var s = max_scale * (1 + pulse_amp * sin(time_accum * pulse_speed));
    image_xscale = s * drop_stacks / 2;
    image_yscale = s * drop_stacks / 2;
    ...
    var s = max_scale * (1 - t);
    image_xscale = max(0.001, s) * drop_stacks / 2;
    image_yscale = image_xscale * drop_stacks / 2;
    image_alpha  = 1 - t;
}
